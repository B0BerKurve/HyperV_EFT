...
bool CMemory::Initialize()
{
	...
	while (!GetProcessID())
	{
		...
		WRAP_IF_DEBUG(
			std::cout << "Waiting for EscapeFromTarkov PID.\n";
		)
	...
	{
		WRAP_IF_DEBUG(
			std::cout << "Failed to get EscapeFromTarkov PID.\n";
		)
	...
	if (!Window)
	{
		...
		WRAP_IF_DEBUG(
			//std::cout << "Failed to get EscapeFromTarkov window.\n";
		)
	...
	{
		WRAP_IF_DEBUG(
			//std::cout << "Failed to get EscapeFromTarkov base address..\n";
		)
...
DWORD CMemory::GetProcessID()
{
	...
	do
	{
		if (wcscmp(pe32.szExeFile, L"EscapeFromTarkov.exe") == 0)
